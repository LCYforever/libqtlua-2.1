...
namespace QtLua {
	...
	void qtlib_register_meta(const QMetaObject *mo, qobject_creator *creator)
	{
		...
		name.replace(':', '_');
		QMetaObjectTable::Instance()._mo_table.insert(name, QMetaObjectWrapper(mo, creator));
	}

	QMetaObjectTable& QMetaObjectTable::Instance()
	{
		static QMetaObjectTable _inst;
		return _inst;
	}

	QModuleTable::QModuleTable()
	{
		for (const meta_object_table_s *me = meta_object_table; me->_mo; me++)
		{
			String name(me->_mo->className());
			name.replace("QtLua::", "");
			qmodule_table_t::Iterator iter = _mo_table.find(me->module);
		...
		_mo_table["QtCore"]->append("Qt", QModuleClassWrapper(&staticQtMetaObject));
		_mo_table["QtWidgets"]->append("QSizePolicy", QModuleClassWrapper(&QtLua::SizePolicy::staticMetaObject));
	}
...
#define REGISTER_QT_MODULE(module, wrapper) \
	ls->set_global("qt."+ module, Value(ls, wrapper))
		...
		qmodule_table_t::Iterator iter = _mo_table.begin();

		for ( ; iter != _mo_table.end(); ++iter)
		{
	...
	{
		ls->set_global("qt.meta", Value(ls, QMetaObjectTable::Instance()));
		QModuleTable::Instance().registerQtModules(ls);
