...
namespace QtLua {
	...
	QModuleClassWrapper::QModuleClassWrapper(const QMetaObject *mo, qobject_creator *creator)
		: QMetaObjectWrapper(mo, creator)
	{
	}

	Value QModuleClassWrapper::meta_index(State *ls, const Value &key)
	{
		const MetaCache &mc = MetaCache::get_meta(_mo);
		String name(key.to_string());
	...
	bool QModuleClassWrapper::support(Value::Operation c) const
	{
		switch (c)
		{
		case Value::OpIndex:
